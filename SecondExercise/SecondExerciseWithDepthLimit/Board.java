...
public class Board{
    ...
    public Board(int rows,int cols,int k){
        ...
        this.score=25;  //declaration of the score(score of the board)
    }
     ...
     * @param col   The collumn we want to place the arg.
     */
    ...
    */
    ...
    /**
     * TO DO A BETTER EVALUATION FUNCTION
     */
    
    /**
     *This method uses Gaussian normal distribution to set weights of every position of the board. 
     *Our computer trys to win by placing moves to cells where is highest probapility of winning(position with superiority)
     *If we want our computer to be a more powerful player and win more often we need to make a better evaluation function
     * @return the score of a state
     */
    public int score() {
        int sumOfBoardElements= 159,sum=0,rowsCounter=0,colsCounter=0,counter=0;
        int[][] evaluationTable= {{4,5,6,8,6,5,4},{5,7,9,11,9,7,5},{6,9,12,14,12,9,6},{6,9,12,14,12,9,6},{5,7,9,11,9,7,5},{4,5,6,8,8,5,4}};
       

        while(true){

            if(this.board[rowsCounter][colsCounter] == 'x')sum+=evaluationTable[rowsCounter][colsCounter];

            if(this.board[rowsCounter][colsCounter] == 'o')sum-=evaluationTable[rowsCounter][colsCounter];

            rowsCounter++;

            if(rowsCounter==this.rows ){
                rowsCounter=0;
                colsCounter++;
                counter++;

                if(counter==this.rows)break;
            }
        }
        return sumOfBoardElements + sum;
    }
    ...
    /**
     * This method is used to evaluate a state. Uses the checkForWinning Method (checks for a winner) and the score()
     * @return The score of specific state
     */
    public int evaluate(){
        int tempWinning=checkForWinning();
       
        if(tempWinning!=0)return tempWinning;   //if we have a winner return the winning score

        return score(); //returns the evaluation score
    }
}